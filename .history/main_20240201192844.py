...
#importamos FASTAIP
from uvicorn.lifespan import LifespanServer
...
async def startup():
    # Cargar la clave privada y el certificado SSL
    ssl_context = ssl.create_default_context(cafile=None)
    ssl_context.load_cert_chain(certfile="cert.pem", keyfile="key.pem")

    # Configurar el servidor HTTPS
    lifespan = LifespanServer(app, port=8000, ssl_context=ssl_context)
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, lifespan="my_lifespan", startup=startup, shutdown=shutdown)
